#!/usr/bin/env python3
"""
Wrapper script for running the nearby business finder.

This script is a convenience wrapper around the 'nearby' command of src/main.py.
It allows users to run the nearby business finder without having to remember the full command syntax.

Usage:
    python scripts/nearby.py --analysis-file FILE [options]

Options:
    --analysis-file FILE Analysis file containing incidents (required)
    --radius METERS      Search radius in meters (default: 1609)
    --max-runtime MIN    Maximum runtime in minutes (default: 5)
    --progress-type TYPE Type of progress indicator (spinner, dots, bar)
"""

import sys
import os
import subprocess

def main():
    """Run the nearby business finder command."""
    # Get the path to the main.py script
    main_script = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'src', 'main.py')
    
    # Build the command
    cmd = [sys.executable, main_script, 'nearby']
    
    # Add any additional arguments
    cmd.extend(sys.argv[1:])
    
    # Run the command
    try:
        subprocess.run(cmd, check=True)
        return 0
    except subprocess.CalledProcessError as e:
        print(f"Error running nearby business finder: {e}", file=sys.stderr)
        return 1

if __name__ == "__main__":
    sys.exit(main())
